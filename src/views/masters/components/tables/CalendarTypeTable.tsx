...
import { calendarTypeData } from '@/data'
import {
    HiOutlineEye,
    HiOutlineMenuAlt4,
...
const CalendarTypeTable = ({ handleEdit }) => {
    ...
    return (
        <Card className="mt-4">
            ...
            <Table>
                ...
                <TBody>
                    {(calendarTypeData || [])?.map((i, index) => (
                        <Tr key={index}>
                            <Td>{index+1}</Td>
                            <Td>{calenderTypeOptions.filter((item)=> item.value === i.type)[0].label}</Td>
                            {/* <Td>
                                <Dropdown
...
export default CalendarTypeTable
