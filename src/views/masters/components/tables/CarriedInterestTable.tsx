...
import { carriedInterestData } from '@/data'
import {
    HiOutlineEye,
    HiOutlineMenuAlt4,
...
const CarriedInterestTable = ({ handleEdit }) => {
    const [searchValue, setSetSearchValue] = useState('')

    const filteredData = carriedInterestData.filter(
        (item) =>
            ...
            <Table>
                ...
                <TBody>
                    {(filteredData || [])?.map((i, index) => (
                        <Tr key={index}>
                            <Td>{index+1}</Td>
                            <Td>{carriedInterestOptions.filter((item)=> item.value === i.value)[0]?.label}</Td>
                            <Td>
                                <Dropdown
...
export default CarriedInterestTable
